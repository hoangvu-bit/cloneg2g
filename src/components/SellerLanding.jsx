...
import productApi, { GAME_MAP, GAME_SLUG_TO_ID, CATEGORY_SLUG_TO_ID, CATEGORY_ID_TO_SLUG } from '../API/ProductApi';
...
export default function SellerLanding({
  ...
  React.useEffect(() => {
    if (currentUser) {
      adminApi.getUsers()
        .then(res => {
          const data = res.data || [];
          const latestUser = data.find(u => (u.mail || '').toLowerCase() === (currentUser.mail || '').toLowerCase());
          if (latestUser) {
        ...
        .catch(err => console.error("Error syncing profile from Supabase:", err));
    }
  }, []);

  // Fetch real listings belonging to current seller from Database
    ...
    if (currentUser) {
      productApi.getAll()
        .then(res => {
          ...
          const myProducts = list.filter(p => 
            p.seller_id === currentUser.id || 
            (p.seller_name && p.seller_name.toLowerCase() === (currentUser.name || '').toLowerCase())
          );
          if (myProducts.length > 0) {
            const mapped = myProducts.map(p => {
              const gameMeta = GAME_MAP[p.game_id] || { id: 'roblox', name: 'Roblox Robux' };
              return {
                id: p.id,
                gameId: gameMeta.id,
                ...
                category: CATEGORY_ID_TO_SLUG[p.category_id] || 'coins',
        ...
        .catch(err => console.error("Error loading seller listings from database:", err));
    }
  };
  ...
  const handleAddListingSubmit = async (e) => {
    e.preventDefault();
    if (!currentUser) {
      if (triggerToast) triggerToast('Vui lòng đăng nhập với tài khoản Người bán (Seller) để đăng sản phẩm!', 'error');
      return;
    }

    if (!addItemName.trim() || !addPrice || !addStock) {
      if (triggerToast) triggerToast('Vui lòng nhập đầy đủ thông tin sản phẩm, giá và số lượng!', 'error');
      return;
    }

    // Clean and validate price & stock numbers
    const cleanPrice = String(addPrice).replace(/[^0-9]/g, '');
    const priceNum = parseInt(cleanPrice, 10);
    if (!priceNum || priceNum < 1000) {
      if (triggerToast) triggerToast('Vui lòng nhập đơn giá hợp lệ (tối thiểu 1.000 ₫)!', 'error');
      return;
    }

    const cleanStock = String(addStock).replace(/[^0-9]/g, '');
    const stockNum = parseInt(cleanStock, 10) || 1;

    const selectedGame = MOCK_GAMES.find(g => g.id === addGameId);
    const gId = GAME_SLUG_TO_ID[addGameId] || parseInt(String(addGameId).replace(/\D/g, ''), 10) || 1;
    const cId = CATEGORY_SLUG_TO_ID[addCategory] || parseInt(String(addCategory).replace(/\D/g, ''), 10) || 1;

    let createdId = `custom-item-${Date.now()}`;
    try {
      const res = await productApi.create({
        seller_id: currentUser ? (parseInt(currentUser.id, 10) || 1) : 1,
        product_name: addItemName.trim(),
        price: priceNum,
        stock_quantity: stockNum,
        description: addDescription || '',
        game_id: gId,
        category_id: cId,
        server: addRegion || 'Vietnam',
        badge: addBadge || 'Người Bán Mới'
      });

      if (res.data?.product?.id) {
        createdId = res.data.product.id;
      }
      if (triggerToast) triggerToast(`Đã lưu thành công sản phẩm "${addItemName}" vào Database! (Mã SP #${createdId})`);
    } catch (err) {
      console.error("Lỗi lưu sản phẩm lên database:", err.response?.data || err.message);
      const errMsg = err.response?.data?.message || err.message;
      if (triggerToast) triggerToast(`Không thể lưu vào Database: ${errMsg}`, 'error');
      return;
    }
    ...
    const newListing = {
      ...
      sellerId: currentUser ? currentUser.id : 1
    };
  ...
  const handleDeleteListing = async (id) => {
    try {
      if (typeof id === 'number' || (!isNaN(id) && !String(id).startsWith('custom-item-'))) {
        await productApi.delete(id);
      }
    } catch (err) {
      console.warn("Lỗi xóa sản phẩm từ database:", err);
    }
  ...
  const saveEditListing = async (id) => {
    const updatedPrice = Number(editPrice);
    const updatedStock = Number(editStock);

    try {
      if (typeof id === 'number' || (!isNaN(id) && !String(id).startsWith('custom-item-'))) {
        await productApi.update(id, { price: updatedPrice, stock_quantity: updatedStock });
      }
    } catch (err) {
      console.warn("Lỗi cập nhật sản phẩm trên database:", err);
    }
    ...
    setEditingId(null);
    if (triggerToast) triggerToast('Cập nhật giá và kho hàng thành công trên Database!');
  };
