...
export default function AdminDashboard({ currentUser, triggerToast }) {
  ...
  // Fetch all seller listings from localStorage
  const fetchListings = () => {
    try {
      const saved = localStorage.getItem('g2g_seller_listings');
      setGlobalListings(saved ? JSON.parse(saved) : []);
    } catch (e) {
  ...
  // Approve Request
  const handleApprove = async (mail) => {
    try {
      const res = await fetch('http://127.0.0.1:5000/admin/approve-seller', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mail })
      });
      if (res.ok) {
        triggerToast(`Đã phê duyệt người bán thành công: ${mail}`);
        fetchUsers();
      } else {
        const data = await res.json();
        triggerToast(data.message || 'Phê duyệt thất bại!', 'error');
      }
    } catch (err) {
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      const res = await fetch('http://127.0.0.1:5000/admin/reject-seller', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mail: rejectMail, reason: rejectReasonText.trim() })
      });
      if (res.ok) {
        triggerToast(`Đã từ chối yêu cầu người bán của: ${rejectMail}`);
        setRejectMail(null);
        fetchUsers();
      } else {
        const data = await res.json();
        triggerToast(data.message || 'Thao tác thất bại!', 'error');
      }
    } catch (err) {
      triggerToast('Lỗi kết nối máy chủ!', 'error');
    }
  ...
  // Direct Role Update
  const handleRoleUpdate = async (mail, role) => {
    try {
      const res = await fetch('http://127.0.0.1:5000/admin/update-role', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mail, role })
      });
      if (res.ok) {
        triggerToast(`Đã chuyển vai trò tài khoản thành ${role === 'seller' ? 'Người bán' : role === 'admin' ? 'Admin' : 'Thành viên'}`);
        fetchUsers();
      } else {
        const data = await res.json();
        triggerToast(data.message || 'Cập nhật thất bại!', 'error');
      }
    } catch (err) {
      triggerToast('Lỗi kết nối máy chủ!', 'error');
    }
  ...
  // Delete product (Global listing management)
  const handleDeleteProduct = (id) => {
    try {
      ...
      if (saved) {
        ...
        setGlobalListings(updated);
        triggerToast('Đã gỡ bỏ sản phẩm thành công khỏi sàn!');
      ...
      console.error(err);
      triggerToast('Gỡ sản phẩm thất bại!', 'error');
    }
  };
              ...
              <tbody>
                {pendingRequests.map(req => (
                  <tr key={req._id}>
                    ...
                    <td>{req.mail}</td>
                    <td><span style={{ color: '#fbbf24', fontWeight: 600 }}>{req.sellerRequestGame}</span></td>
                    <td style={{ maxWidth: '300px', whiteSpace: 'normal', fontSize: '13px', color: '#cbd5e1' }}>
                      {req.sellerRequestExperience || 'Không có mô tả.'}
                    </td>
                    <td>
                      <button className="admin-btn approve" onClick={() => handleApprove(req.mail)}>Duyệt</button>
                      <button className="admin-btn reject" onClick={() => openRejectDialog(req.mail)}>Từ chối</button>
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                <tr key={u._id}>
                  ...
                  <td>
                    <select
                      ...
                      value={u.role}
                      onChange={(e) => handleRoleUpdate(u.mail, e.target.value)}
                      disabled={u.mail === 'admin@gmail.com'} // disable changing main admin role
                    >
        ...
        <div className="reject-modal-overlay">
          <div className="reject-modal-box">
            ...
            <p style={{ fontSize: '13px', color: '#9ea2a9', margin: '0 0 12px' }}>
              Nhập lý do từ chối cho tài khoản: <strong>{rejectMail}</strong>. Người dùng sẽ nhận được phản hồi này.
            </p>
            <textarea
