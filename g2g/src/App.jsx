...
function App() {
  ...
  const handleLoginSubmit = (e) => {
    ...
    triggerToast(`Đăng nhập thành công! Chào mừng ${loginEmail}`);
    setActiveModal(null);
    setLoginEmail('');
    setLoginPassword('');
  };
  ...
  const filteredSuggestions = searchQuery ? MOCK_GAMES.filter(g => 
  ...
  const filteredCategoryItems = flatCategoryItems.filter(item => {
    ...
    const matchesMinPrice = filterMinPrice === '' || itemPrice >= parseInt(filterMinPrice);
    const matchesMaxPrice = filterMaxPrice === '' || itemPrice <= parseInt(filterMaxPrice);
    return matchesGame && matchesMinPrice && matchesMaxPrice;
  });

  // Apply Sort orders
  const sortedFilteredCategoryItems = [...filteredCategoryItems].sort((a, b) => {
    ...
    <div className="g2g-clone-app">
        ...
        <div className="container navbar">
          ...
          <div className="nav-search-bar-wrapper">
            <div className="nav-search-bar">
              ...
              />
              {searchQuery && (
                <span className="clear-search-btn" onClick={() => setSearchQuery('')}>×</span>
              )}
                ...
                {searchQuery ? (
                  filteredSuggestions.length > 0 ? (
                    filteredSuggestions.map(game => (
                      <div key={game.id} className="suggestion-item" onClick={() => navigateToCatalog(game)}>
                        <div className="suggestion-game-info">
                          <span className="suggestion-icon" style={{ background: game.color }}>{game.textIcon}</span>
                          <div>
                        ...
                        <div className="suggestion-chips">
                          <span className="chip" onClick={(e) => { e.stopPropagation(); navigateToCatalog(game); }}>Ưu đãi</span>
                          <span className="chip" onClick={(e) => { e.stopPropagation(); navigateToCatalog(game); }}>Nạp thẻ</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="no-suggestion">Không tìm thấy game nào phù hợp với "{searchQuery}"</div>
                  )
                ) : (
                  <div className="default-suggestions">
                    ...
                    <div className="trending-chips-grid">
                      {MOCK_GAMES.slice(0, 4).map(g => (
                        <div key={g.id} className="trending-chip" onClick={() => navigateToCatalog(g)}>
          ...
          <div className="nav-actions">
            <button className="seller-link btn btn-text" onClick={() => setActiveModal('seller')}>
              Trở thành người bán
              <span className="badge badge-success" style={{ marginLeft: '6px' }}>Free</span>
            </button>

            {/* Shopping Cart Trigger */}
            <button className="cart-trigger-btn" onClick={() => setIsCartOpen(true)} title="Giỏ Hàng">
              <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4H6z"></path>
                <path d="M3 6h18M16 10a4 4 0 0 1-8 0"></path>
              </svg>
              {cart.length > 0 && <span className="cart-count-badge">{cart.reduce((sum, i) => sum + i.qty, 0)}</span>}
            </button>

            {/* Chat Trigger */}
            <button className="chat-trigger-btn" onClick={() => { setShowChatDrawer(true); if (chats.length > 0 && activeChatId === null) setActiveChatId(chats[0].id); }} title="Tin nhắn">
              <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <nav className="sub-navbar">
        <div className="container sub-navbar-container">
          <span className={`sub-nav-item ${selectedCategory === 'all' && currentView === 'category-catalog' ? 'active' : ''}`} onClick={() => navigateToCategory('all')}>
            🌐 Tất cả danh mục
          </span>
          <span className={`sub-nav-item ${selectedCategory === 'coins' && currentView === 'category-catalog' ? 'active' : ''}`} onClick={() => navigateToCategory('coins')}>
            🪙 Tiền tệ (Coins)
          </span>
          <span className={`sub-nav-item ${selectedCategory === 'accounts' && currentView === 'category-catalog' ? 'active' : ''}`} onClick={() => navigateToCategory('accounts')}>
            👤 Tài khoản VIP
          </span>
          <span className={`sub-nav-item ${selectedCategory === 'cards' && currentView === 'category-catalog' ? 'active' : ''}`} onClick={() => navigateToCategory('cards')}>
            💳 Thẻ Game / Gift Card
          </span>
          <span className={`sub-nav-item ${selectedCategory === 'boosting' && currentView === 'category-catalog' ? 'active' : ''}`} onClick={() => navigateToCategory('boosting')}>
            ⚡ Cày thuê (Boosting)
          </span>
        </div>
      </nav>
            ...
            <section className="categories-section" style={{ paddingTop: '32px', paddingBottom: '32px' }}>
              <div className="container">
                ...
                <div className="g2g-category-grid">
                  ...
                  <div className="g2g-category-large-card" onClick={() => triggerToast('Dịch vụ Game Coaching sẽ được ra mắt sớm!')}>
                  ...
                  <div className="g2g-category-large-card" onClick={() => triggerToast('Khám phá các đại sứ GamePal ở mục bên dưới!')}>
                    <span className="category-beta-badge">Beta</span>
                    <div className="category-large-icon">
                      <svg width="44" height="44" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                  ...
                  <div className="g2g-category-horizontal-card" onClick={() => triggerToast('Dịch vụ nạp Vật phẩm đang chuẩn bị cập nhật!')}>
                    <div className="category-horiz-icon">📦</div>
                  ...
                  <div className="g2g-category-horizontal-card" onClick={() => triggerToast('Thị trường trang phục/Skin đang bảo trì!')}>
                    <div className="category-horiz-icon">🛡️</div>
                    <span className="category-horiz-name">Skin</span>
                  </div>

                  <div className="g2g-category-horizontal-card" onClick={() => triggerToast('Dịch vụ Nạp tiền điện thoại đang liên kết nhà mạng!')}>
                    <div className="category-horiz-icon">📱</div>
                    <span className="category-horiz-name">Nạp tiền điện thoại</span>
                  </div>

                  <div className="g2g-category-horizontal-card" onClick={() => triggerToast('Thị trường bản quyền phần mềm đang liên kết!')}>
                    <div className="category-horiz-icon">💻</div>
                    <span className="category-horiz-name">Phần mềm &amp; Ứng dụng</span>
                  </div>

                  <div className="g2g-category-horizontal-card" onClick={() => triggerToast('Các gói nạp thẻ thanh toán visa/mastercard đang cập nhật!')}>
                    <div className="category-horiz-icon">💳</div>
              ...
              <div className="container">
                ...
                <div className="flash-sale-grid">
                  {FLASH_SALE_ITEMS.map(item => {
                    ...
                    const percentSold = Math.floor(((item.totalStock - item.stockLeft) / item.totalStock) * 100);
                    
                    return (
                      <div key={item.id} className="flash-card" onClick={() => navigateToDetail(gameObj, itemObj)}>
                        ...
                        <div className="flash-info">
                          ...
                          <div className="flash-price-row">
                            <span className="original-price">{item.originalPrice.toLocaleString('vi-VN')}₫</span>
                            <span className="sale-price">{item.salePrice.toLocaleString('vi-VN')}₫</span>
                          </div>
                          ...
                          <div className="flash-stock-progress">
                            ...
                            <div className="progress-text-row justify-between">
                              <span>Đã bán: <strong>{percentSold}%</strong></span>
                              <span>Còn lại: <strong style={{ color: 'var(--brand-red)' }}>{item.stockLeft} thẻ</strong></span>
                            </div>
                          </div>

                          <button className="btn btn-primary btn-sm flash-buy-btn" style={{ width: '100%' }}>
                            Giật Deal Ngay
                          </button>
                        </div>
                      </div>
                ...
                <div className="coaching-row-wrapper">
                  <div className="coaching-header-row justify-between">
                    <div className="coaching-header-left">
                      ...
                      <p className="coaching-section-subtitle">
                        Muốn trở nên giỏi hơn? Đặt huấn luyện viên chuyên gia để phân tích lối chơi của bạn và mở khóa tiềm năng thực sự của bạn.
                      </p>
                    </div>
                    <span className="explore-all-link" onClick={() => triggerToast('Dịch vụ Game Coaching sẽ ra mắt danh sách đầy đủ huấn luyện viên sớm!')}>
                  ...
                  <div className="coaching-circle-grid">
                    {COACHING_PARTNERS.map(coach => (
                      <div key={coach.id} className="coaching-circle-item" onClick={() => openChatWithPartner(coach.name, 'Coaching')}>
                        <div className="coaching-avatar-wrapper">
                          <img src={coach.avatarImage} alt={coach.name} className="coaching-avatar-img" />
                ...
                <div className="coaching-row-wrapper" style={{ marginTop: '48px' }}>
                  <div className="coaching-header-row justify-between">
                    <div className="coaching-header-left">
                      ...
                      <p className="coaching-section-subtitle">
                        Chỉ muốn có thời gian vui vẻ? Hợp tác với GamePal đã được xác minh để có trải nghiệm chơi game tuyệt vời, không áp lực.
                      </p>
                    </div>
                    <span className="explore-all-link" onClick={() => triggerToast('Dịch vụ GamePal sẽ mở rộng danh sách đại sứ sớm!')}>
                      Khám phá tất cả <span className="arrow">&gt;</span>
                    </span>
                  </div>
                  ...
                  <div className="coaching-circle-grid">
                    {GAMEPAL_AVATARS.map(pal => (
                      <div key={pal.id} className="coaching-circle-item" onClick={() => openChatWithPartner(pal.name, 'GamePal Companion')}>
                        <div className="coaching-avatar-wrapper">
              ...
              <div className="container">
                ...
                <div className="g2g-trending-tabs-wrapper">
                  <div className="g2g-trending-tabs-container">
                    <span className={`g2g-trending-tab-link ${activeCategory === 'cards' ? 'active' : ''}`} onClick={() => setActiveCategory('cards')}>
                      Thẻ quà tặng
                    </span>
                    <span className={`g2g-trending-tab-link ${activeCategory === 'all' ? 'active' : ''}`} onClick={() => setActiveCategory('all')}>
                      Trò chơi
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Hãy cuộn lên phía trên để xem dịch vụ Game Coaching!')}>
                      Game Coaching
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Hãy cuộn lên phía trên để xem dịch vụ GamePal!')}>
                      GamePal
                    </span>
                    <span className={`g2g-trending-tab-link ${activeCategory === 'coins' ? 'active' : ''}`} onClick={() => setActiveCategory('coins')}>
                      Xu Game
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Thị trường Vật phẩm đang cập nhật xu hướng!')}>
                      Vật phẩm
                    </span>
                    <span className={`g2g-trending-tab-link ${activeCategory === 'accounts' ? 'active' : ''}`} onClick={() => setActiveCategory('accounts')}>
                      Tài khoản Game
                    </span>
                    <span className={`g2g-trending-tab-link ${activeCategory === 'boosting' ? 'active' : ''}`} onClick={() => setActiveCategory('boosting')}>
                      Cày thuê
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Thị trường Trang phục/Skin đang cập nhật xu hướng!')}>
                      Skin
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Giao dịch nạp tiền điện thoại đang liên kết đại lý!')}>
                      Nạp tiền điện thoại
                    </span>
                    <span className="g2g-trending-tab-link" onClick={() => triggerToast('Phần mềm bản quyền đang liên kết đại lý!')}>
                      Phần mềm &amp; Ứng dụng
                    </span>
                  </div>
                  
                  {/* Arrow Indicator */}
                  <span className="g2g-trending-tab-arrow" onClick={() => triggerToast('Cuộn ngang hoặc kéo để xem thêm danh mục xu hướng!')}>
                    &gt;
                  </span>
                </div>
                  ...
                  return (
                    <div className="g2g-trending-grid">
                      {items.map((item, index) => (
                        <div key={index} className="g2g-trending-gold-card" onClick={() => handleTrendingCardClick(item)}>
                  ...
                  <div className="promo-card">
                    ...
                    <div>
                      ...
                      <p className="promo-description">
                        Chia sẻ liên kết G2G của bạn và kiếm tới 5% hoa hồng trên mỗi giao dịch thành công. Rút tiền nhanh chóng và trực quan.
                      </p>
                    ...
                    <button className="btn btn-primary" style={{ alignSelf: 'flex-start' }} onClick={() => triggerToast('Hệ thống đối tác đang chuẩn bị ra mắt vào tháng tới!')}>
                      Bắt đầu kiếm tiền
                    </button>
                  </div>

                  <div className="promo-card" style={{ background: 'linear-gradient(135deg, rgba(255, 78, 58, 0.08) 0%, rgba(245, 158, 11, 0.05) 100%)' }}>
                    ...
                    <div>
                      ...
                      <p className="promo-description">
                        Giao dịch được bảo vệ 100% bằng cơ chế ký quỹ và bảo vệ tài khoản cho cả người mua và người bán.
                      </p>
                      <ul className="protection-list">
                        <li><span>✓</span> Cam kết hoàn tiền 100% khi xảy ra sự cố tài khoản.</li>
                        <li><span>✓</span> Giải quyết tranh chấp công bằng 24/7.</li>
                        <li><span>✓</span> Bảo vệ thông tin thanh toán tuyệt đối qua cổng SSL.</li>
                      </ul>
                    </div>
                    <button className="btn btn-secondary" style={{ alignSelf: 'flex-start' }} onClick={() => triggerToast('Bảo hiểm GamerProtect được áp dụng mặc định cho mọi giao dịch!')}>
                      Tìm hiểu thêm
                    </button>
                  </div>
                </div>
            ...
            <div className="container">
              ...
              <div className="directory-hero-banner">
                ...
                <div className="directory-hero-content">
                  ...
                  <p>Đặt huấn luyện viên đẳng cấp để phân tích lối chơi, nâng tầm trình độ và leo hạng thần tốc!</p>
                </div>
              </div>
              ...
              <div className="coaches-grid-list">
                {COACHING_PARTNERS.filter(coach => {
                  const matchesSearch = coach.name.toLowerCase().includes(coachingSearchQuery.toLowerCase());
                  const matchesGame = coachingGameFilter === 'all' || 
                    (coachingGameFilter === 'Valorant' && (coach.id === 'c1' || coach.id === 'c5')) || 
                    (coachingGameFilter === 'Dota 2' && (coach.id === 'c3' || coach.id === 'c4')) ||
                    (coachingGameFilter === 'League of Legends' && (coach.id === 'c2' || coach.id === 'c6'));
                  ...
                  <div key={coach.id} className="coach-profile-card">
                    <div className="coach-card-header">
                      <div className="coach-avatar-container">
                        ...
                        {coach.online && <span className="online-indicator-dot"></span>}
                      </div>
                      <div className="coach-brief-info">
                        <h4>{coach.name}</h4>
                        <span className="coach-game-tag">
                          {coach.id === 'c1' || coach.id === 'c5' ? 'Valorant Pro Coach' :
                           coach.id === 'c3' || coach.id === 'c4' ? 'Dota 2 Immortals' : 'LMHT Thách Đấu'}
                        </span>
                        <div className="coach-rating-stars">⭐ 4.9 (140+ đánh giá)</div>
                      </div>
                    </div>

                    <div className="coach-desc-body">
                      Chuyên phân tích lối chơi macro, sửa tư duy di chuyển, hướng dẫn quản lý lính và tối ưu hóa bể tướng leo rank hiệu quả.
                    </div>
                    ...
                    <div className="coach-card-footer justify-between">
                      ...
                      <button className="btn btn-primary btn-sm" onClick={() => openChatWithPartner(coach.name, 'Coaching')}>
                        💬 Đặt Lịch &amp; Chat
                      </button>
                    </div>
                  </div>
            ...
            <div className="container">
              ...
              <div className="directory-hero-banner gamepal-hero-bg">
                ...
                <div className="directory-hero-content">
                  ...
                  <p>Trải nghiệm chơi game vui vẻ, không áp lực! Tìm bạn đồng hành nói chuyện dễ thương, chơi game giỏi.</p>
                </div>
              </div>
              ...
              <div className="coaches-grid-list">
                {GAMEPAL_AVATARS.filter(pal => {
                  const matchesSearch = pal.name.toLowerCase().includes(gamepalSearchQuery.toLowerCase());
                  const matchesGame = gamepalGameFilter === 'all' ||
                    (gamepalGameFilter === 'Valorant' && (pal.id === 'g1' || pal.id === 'g4')) ||
                    (gamepalGameFilter === 'League of Legends' && (pal.id === 'g2' || pal.id === 'g6')) ||
                    (gamepalGameFilter === 'Genshin Impact' && (pal.id === 'g3' || pal.id === 'g5'));
                  return matchesSearch && matchesGame;
                }).map(pal => (
                  <div key={pal.id} className="coach-profile-card">
                    <div className="coach-card-header">
                      <div className="coach-avatar-container">
                        ...
                        {pal.online && <span className="online-indicator-dot"></span>}
                      </div>
                      <div className="coach-brief-info">
                        <h4>{pal.name}</h4>
                        <span className="coach-game-tag">
                          {pal.id === 'g1' || pal.id === 'g4' ? 'Valorant • Mic On' :
                           pal.id === 'g2' || pal.id === 'g6' ? 'LMHT • Vui Vẻ' : 'Genshin • Co-op'}
                        </span>
                        <div className="coach-rating-stars">⭐ 5.0 (80+ review)</div>
                      </div>
                    </div>

                    <div className="coach-desc-body">
                      Có mic nói chuyện dễ nghe, kỹ năng cá nhân khá tốt, sẵn sàng gánh tạ, tấu hài giúp bạn có trải nghiệm chơi game thư giãn nhất!
                    </div>
                    ...
                    <div className="coach-card-footer justify-between">
                      ...
                      <button className="btn btn-outline btn-sm" onClick={() => openChatWithPartner(pal.name, 'GamePal')}>
                        💬 Chat Ngay
                      </button>
                    </div>
                  </div>
            ...
            <div className="container">
              ...
              <div className="topup-layout-grid">
                ...
                <div className="topup-form-container">
                  ...
                  <p className="topup-form-subtitle">Điền số điện thoại, chọn nhà mạng và mệnh giá để nạp chiết khấu rẻ nhất thị trường.</p>

                  <div className="form-group" style={{ marginTop: '24px' }}>
                    <label className="topup-input-label">Số điện thoại nhận tiền</label>
                    <input 
                      type="tel" 
                  ...
                  <div className="form-group">
                    ...
                    <div className="operator-selector-pills">
                      <div className={`operator-pill ${topupOperator === 'viettel' ? 'active' : ''}`} onClick={() => setTopupOperator('viettel')}>
                        <span className="operator-name">Viettel</span>
                        <span className="operator-discount">-4%</span>
                      </div>
                      <div className={`operator-pill ${topupOperator === 'mobifone' ? 'active' : ''}`} onClick={() => setTopupOperator('mobifone')}>
                        <span className="operator-name">Mobifone</span>
                        <span className="operator-discount">-3%</span>
                      </div>
                      <div className={`operator-pill ${topupOperator === 'vinaphone' ? 'active' : ''}`} onClick={() => setTopupOperator('vinaphone')}>
                  ...
                  <div className="form-group">
                    <label className="topup-input-label">Chọn mệnh giá nạp</label>
                    <div className="topup-denoms-grid">
                      {[10000, 20000, 50000, 100000, 200000, 500000].map(val => (
                        ...
                        >
                          <div className="denom-face-value">{(val).toLocaleString('vi-VN')}₫</div>
                          <div className="denom-selling-price">
                            Giá bán: {Math.floor(val * (topupOperator === 'viettel' ? 0.96 : topupOperator === 'mobifone' ? 0.97 : 0.965)).toLocaleString('vi-VN')}₫
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <button 
                    ...
                    onClick={() => {
                      if (!topupPhone || topupPhone.length < 10) {
                        triggerToast('Vui lòng nhập đúng định dạng số điện thoại 10 số!');
                        return;
                      }
                      const finalPrice = Math.floor(topupAmount * (topupOperator === 'viettel' ? 0.96 : topupOperator === 'mobifone' ? 0.97 : 0.965));
                      const topupItem = {
                      ...
                      setIsCartOpen(true);
                      triggerToast('Đã thêm hóa đơn nạp điện thoại vào Giỏ hàng!');
                    }}
                  >
                ...
                <div className="topup-guide-container">
                  ...
                  <div className="guide-card-content">
                    ...
                    <ul className="guide-steps-list">
                      <li>
                        <strong>Giao hàng tự động:</strong> Hệ thống tự động bắn tiền trực tiếp vào tài khoản thuê bao trả trước/trả sau trong 5 phút.
                      </li>
                      <li>
                        <strong>GamerProtect bảo vệ:</strong> Cam kết hoàn tiền 100% nếu xảy ra lỗi hệ thống hoặc không nhận được tiền nạp.
                      </li>
                      <li>
                        <strong>Hỗ trợ trực tuyến:</strong> Đội ngũ chăm sóc khách hàng hỗ trợ giải quyết thắc mắc về số thuê bao 24/7.
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            <div className="container">
              ...
              <div className="breadcrumbs">
                ...
                <span className="active">
                  {selectedCategory === 'all' ? 'Tất cả sản phẩm' :
                   ...
                   selectedCategory === 'accounts' ? 'Tài khoản VIP' :
                   selectedCategory === 'cards' ? 'Thẻ game / Gift Card' : 'Cày thuê (Boosting)'}
                </span>
              </div>
              ...
              <div className="brand-search-tab-bar">
                <div className="brand-search-input-wrapper">
                  <svg className="search-icon-mini" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                    <circle cx="11" cy="11" r="8"></circle>
                    <path d="M21 21l-4.35-4.35"></path>
                  </svg>
                  <input 
              ...
              {(() => {
                const filteredGames = MOCK_GAMES.filter(game => {
                  const matchesMainCategory = selectedCategory === 'all' || game.category === selectedCategory;
                  const matchesSearch = game.name.toLowerCase().includes(brandSearchQuery.toLowerCase());
                ...
                if (filteredGames.length === 0) {
                  return (
                    <div className="empty-catalog-results">
                      ...
                      <p>Vui lòng nhập lại tên game hoặc thương hiệu khác trong thanh tìm kiếm bên trên.</p>
                      <button className="btn btn-primary btn-sm" onClick={() => { setBrandSearchQuery(''); setActiveBrandTab('all'); }}>Đặt lại bộ lọc</button>
                    </div>
                  );
                }
                ...
                return (
                  <>
                    ...
                    <div className="directory-section-container">
                      ...
                      <div className="brand-trending-grid">
                        {trendingGames.map(game => {
                          const totalOffers = game.items.reduce((sum, item) => sum + item.offers, 0);
                          return (
                            <div key={game.id} className="brand-trending-block" onClick={() => navigateToCatalog(game)}>
                    ...
                    <div className="directory-section-container" style={{ marginTop: '40px' }}>
                      <h3 className="directory-section-title">Tất cả thương hiệu cho {
                        selectedCategory === 'all' ? 'Trò chơi' :
                        selectedCategory === 'coins' ? 'Tiền tệ Game (Coins)' :
                        selectedCategory === 'accounts' ? 'Tài khoản VIP' :
                        selectedCategory === 'cards' ? 'Thẻ game / Gift Card' : 'Cày thuê (Boosting)'
                      }</h3>
                      <div className="brand-all-grid">
                        {allGames.map(game => {
                          const totalOffers = game.items.reduce((sum, item) => sum + item.offers, 0);
                          return (
                            <div key={game.id} className="brand-all-block" onClick={() => navigateToCatalog(game)}>
            ...
            <div className="container">
              ...
              <div className="catalog-hero-row justify-between">
                <h1 className="catalog-hero-title">
                  {selectedGame.name} {selectedCategory === 'coins' ? 'Coins' :
                                       selectedCategory === 'boosting' ? 'Boosting' :
                                       selectedCategory === 'accounts' ? 'Accounts' : 'Gift Cards'}
                </h1>
                <button className="btn btn-secondary share-btn-box" onClick={() => triggerToast('Đường liên kết chia sẻ đã được sao chép vào bộ nhớ tạm!')}>
                  <span>🔗 Chia sẻ</span>
                </button>
              </div>
              ...
              <div className="catalog-service-switcher-wrapper">
                <div className={`service-switcher-circle ${selectedCategory === 'coins' ? 'active' : ''}`} onClick={() => setSelectedCategory('coins')}>
                ...
                <div className={`service-switcher-circle ${selectedCategory === 'boosting' ? 'active' : ''}`} onClick={() => setSelectedCategory('boosting')}>
                  <div className="service-circle-icon">🔥</div>
                  <span className="service-circle-name">Cày thuê</span>
                  <span className="service-circle-count">(36,528)</span>
                </div>
                <div className={`service-switcher-circle ${selectedCategory === 'cards' ? 'active' : ''}`} onClick={() => setSelectedCategory('cards')}>
                  <div className="service-circle-icon">💳</div>
                  <span className="service-circle-name">Mã kích hoạt</span>
                  <span className="service-circle-count">(28)</span>
                </div>
                <div className={`service-switcher-circle ${selectedCategory === 'coaching' ? 'active' : ''}`} onClick={() => setSelectedCategory('coaching')}>
                  <div className="service-circle-icon">🎮</div>
                  <span className="service-circle-name">Coaching</span>
                  <span className="service-circle-count">(16)</span>
                </div>
                <div className={`service-switcher-circle ${selectedCategory === 'gamepal' ? 'active' : ''}`} onClick={() => setSelectedCategory('gamepal')}>
                  <div className="service-circle-icon">👥</div>
                  <span className="service-circle-name">GamePal</span>
                  <span className="service-circle-count">(10)</span>
                </div>
                <div className={`service-switcher-circle ${selectedCategory === 'items' ? 'active' : ''}`} onClick={() => setSelectedCategory('items')}>
                  <div className="service-circle-icon">📦</div>
                  <span className="service-circle-name">Vật phẩm</span>
                  <span className="service-circle-count">(16,746)</span>
                </div>
                <div className={`service-switcher-circle ${selectedCategory === 'accounts' ? 'active' : ''}`} onClick={() => setSelectedCategory('accounts')}>
                  <div className="service-circle-icon">👤</div>
              ...
              <div className="popular-tags-row">
                ...
                <span className="popular-tag-chip" onClick={() => setCatalogSearchQuery('game time')}>game time</span>
                <span className="popular-tag-chip" onClick={() => setCatalogSearchQuery(selectedGame.name.toLowerCase())}>{selectedGame.name.toLowerCase()}</span>
                <span className="popular-tag-chip" onClick={() => setCatalogSearchQuery('midnight')}>midnight</span>
                <span className="popular-tag-chip" onClick={() => setCatalogSearchQuery('time')}>time</span>
                <span className="popular-tag-chip" onClick={() => setCatalogSearchQuery('gold')}>gold</span>
              </div>
              ...
              {(() => {
                const filteredCatalogItems = selectedGame.items.filter(item => {
                  ...
                  const matchesRegion = catalogRegionFilter === 'all' || item.region.toLowerCase() === catalogRegionFilter.toLowerCase();
                ...
                return (
                  <>
                    <div className="catalog-results-header justify-between">
                      <span className="results-count-label">Khoảng {sortedCatalogItems.length} kết quả</span>
                    ...
                    {sortedCatalogItems.length > 0 ? (
                      <div className="catalog-packages-grid">
                        {sortedCatalogItems.map(item => (
                          <div key={item.id} className="catalog-package-card" onClick={() => navigateToDetail(selectedGame, item)}>
                            ...
                            <div className="package-card-footer justify-between">
                              ...
                              <span className="package-price-text">từ {item.price.toLocaleString('vi-VN')}₫</span>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="empty-catalog-results">
                        ...
                        <button className="btn btn-primary btn-sm" onClick={() => { setCatalogSearchQuery(''); setCatalogRegionFilter('all'); }}>Đặt lại bộ lọc</button>
                      </div>
                    )}
                  </>
                );
                  ...
                  <div className="selected-item-display">
                    ...
                    <div className="item-display-info">
                      ...
                      <div className="item-meta-tags">
                        <span>Khu vực: <strong>{selectedItem.region}</strong></span>
                        <span className="bullet">•</span>
                        <span>Hình thức: <strong>Tự động gửi/Bàn giao ngay</strong></span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="detail-tab-pane">
                      {activeDetailTab === 'description' ? (
                        <div className="desc-content">
                          <p>Chào mừng bạn đến với đại lý phân phối chính thức của chúng tôi trên G2G Marketplace. Dưới đây là các thông tin quan trọng bạn cần nắm rõ:</p>
                          <ul>
                            <li><strong>Bàn giao tự động:</strong> Sản phẩm được gửi trực tiếp qua hệ thống tin nhắn hoặc email liên kết của bạn ngay sau khi hoàn tất thanh toán.</li>
                            <li><strong>Bảo hành GamerProtect:</strong> Bảo hiểm hoàn trả 100% số tiền nếu mã thẻ có lỗi hoặc tài khoản có vấn đề tranh chấp do lỗi từ người bán trong vòng 7 ngày.</li>
                            <li><strong>Lưu ý:</strong> Vui lòng không tiết lộ thông tin mã nạp, mã OTP hoặc thông tin mật khẩu tài khoản cho bất kỳ ai khác ngoại trừ biểu mẫu giao dịch chính thức.</li>
                          </ul>
                        </div>
                      ) : (
                        <div className="reviews-tab-pane">
                          <div className="reviews-summary-score justify-between">
                            <div>
                              ...
                              <span className="reviews-count-label">Phản hồi tích cực đạt 99.8% từ khách hàng</span>
                            </div>
                            <div className="reviews-recommend">
                              🚀 <strong>Khuyên dùng:</strong> 99% khách hàng rất hài lòng về tốc độ bàn giao của người bán này.
                            </div>
                          </div>
                              ...
                              <div key={rev.id} className="review-item">
                                <div className="review-item-header justify-between">
                                  <div className="buyer-profile">
                                    <span className="avatar-letter">{rev.user.slice(0, 2).toUpperCase()}</span>
                                    <div>
                  ...
                  <div className="purchase-console-card">
                    ...
                    <div className="seller-box-summary">
                      ...
                      <div className="seller-box-info">
                        ...
                        <div className="seller-box-rating">
                          ...
                          <span className="reviews-count">({selectedSeller.reviews.toLocaleString()} reviews)</span>
                        </div>
                    ...
                    <div className="console-price-row">
                      ...
                      <span className="console-price-value">
                        {Math.floor(selectedItem.price * selectedSeller.multiplier).toLocaleString('vi-VN')}₫
                      </span>
                    </div>
                    ...
                    <div className="console-qty-row">
                      ...
                      <div className="qty-picker">
                        <button className="qty-btn" onClick={() => setDetailQuantity(prev => Math.max(1, prev - 1))}>-</button>
                        <input 
                          type="number" 
                          className="qty-input" 
                        ...
                        />
                        <button className="qty-btn" onClick={() => setDetailQuantity(prev => prev + 1)}>+</button>
                      </div>
                    </div>
                    ...
                    <div className="console-total-row">
                      ...
                      <span className="total-value">
                        {(Math.floor(selectedItem.price * selectedSeller.multiplier) * detailQuantity).toLocaleString('vi-VN')}₫
                      </span>
                    </div>
                    ...
                    <div className="console-actions">
                      <button 
                        ...
                        onClick={() => handleBuyNow(selectedItem, selectedGame, selectedSeller, detailQuantity)}
                      >
                        Mua Ngay
                      </button>
                      <button 
                        ...
                        onClick={() => handleAddToCart(selectedItem, selectedGame, selectedSeller, detailQuantity)}
                    ...
                    <button 
                      ...
                      style={{ width: '100%', marginTop: '12px', borderRadius: '8px', padding: '10px', fontSize: '13px' }}
                  ...
                  <table className="compare-table">
                    ...
                    <tbody>
                      {MOCK_SELLERS.map(seller => (
                        <tr key={seller.id} className={selectedSeller.id === seller.id ? 'active-row' : ''}>
                          <td>
                            <div className="compare-seller-info">
                              <span className="compare-seller-avatar">{seller.name.slice(0, 2).toUpperCase()}</span>
                              <div>
                                <div className="compare-seller-name" onClick={() => setSelectedSeller(seller)}>
                                  {seller.name}
                                </div>
                          ...
                          <td><span className="text-success">{seller.successRate}</span></td>
                          <td><span>{seller.speed}</span></td>
                          <td><span>{seller.stock} sản phẩm</span></td>
                          <td>
                            <span className="compare-price-val">
                              {Math.floor(selectedItem.price * seller.multiplier).toLocaleString('vi-VN')}₫
                            </span>
                          </td>
                        ...
                        {activePaymentTab === 'momo' && (
                          <div className="qr-payment-container">
                            <div className="qr-box">
                              ...
                              <div className="qr-amount">Tổng tiền: {(getCartTotal() + 15000).toLocaleString('vi-VN')}₫</div>
                            </div>
                            <div className="qr-instructions">
                              <h5>Quét Mã QR MoMo</h5>
                              <ol>
                                <li>Mở ứng dụng MoMo trên điện thoại di động của bạn.</li>
                                <li>Chọn tính năng <strong>Quét mã</strong> và quét mã QR ở bên cạnh.</li>
                                <li>Nhập đúng mã nội dung thanh toán: <strong>G2G-{Math.floor(10000 + Math.random() * 90000)}</strong></li>
                                <li>Nhấn nút <strong>Xác nhận đã chuyển tiền</strong> bên dưới sau khi hoàn thành.</li>
                              </ol>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="qr-payment-container">
                            <div className="qr-box">
                              <div className="virtual-qr-code" style={{ borderColor: '#0070e0' }}>
                                <div className="qr-corner top-left" style={{ borderColor: '#0070e0' }}></div>
                                <div className="qr-corner top-right" style={{ borderColor: '#0070e0' }}></div>
                                <div className="qr-corner bottom-left" style={{ borderColor: '#0070e0' }}></div>
                                <div className="qr-corner bottom-right" style={{ borderColor: '#0070e0' }}></div>
                                <div className="qr-logo-momo" style={{ background: '#0070e0' }}>Zalo</div>
                                <div className="qr-matrix-dots"></div>
                              </div>
                              <div className="qr-amount">Tổng tiền: {(getCartTotal() + 12000).toLocaleString('vi-VN')}₫</div>
                            </div>
                            <div className="qr-instructions">
                              ...
                              <ol>
                                ...
                                <li>Nội dung chuyển khoản mặc định: <strong>G2G-ZALO-{Math.floor(10000 + Math.random() * 90000)}</strong></li>
                              </ol>
                            </div>
                          </div>
                        )}
                          ...
                          <div className="banking-instructions">
                            ...
                            <div className="banking-details-grid">
                              ...
                              <div className="banking-field">
                                ...
                                <span className="value copy-value">999920268888 <span className="copy-icon" onClick={() => triggerToast('Đã copy số tài khoản!')}>📋</span></span>
                              </div>
                              <div className="banking-field">
                              ...
                              <div className="banking-field">
                                ...
                                <span className="value">{(getCartTotal() + 10000).toLocaleString('vi-VN')}₫</span>
                              </div>
                              <div className="banking-field" style={{ gridColumn: 'span 2' }}>
                                <span className="label">Nội dung chuyển tiền:</span>
                                <span className="value copy-value" style={{ color: 'var(--brand-red)', fontWeight: 'bold' }}>
                                  G2G BANKING {Math.floor(100000 + Math.random() * 900000)}
                                  <span className="copy-icon" onClick={() => triggerToast('Đã copy nội dung!')}>📋</span>
                                </span>
                              </div>
                            </div>
                            <div className="note-alert">
                              ⚠️ Hệ thống tự động cộng tiền trong vòng 10 giây sau khi nhận được chuyển khoản ngân hàng chính xác nội dung.
                            </div>
                          </div>
                        )}
                  ...
                  <div className="checkout-card">
                      ...
                      {cart.map(item => (
                        <div key={item.cartId} className="checkout-item-row">
                          <div className="checkout-item-details">
                            <span className="checkout-item-icon" style={{ background: item.color }}>{item.textIcon}</span>
                            <div>
                          ...
                          <span className="checkout-item-price">{(item.price * item.qty).toLocaleString('vi-VN')}₫</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="checkout-totals">
                      ...
                      <div className="total-row-sub">
                        ...
                        <span>
                          {activePaymentTab === 'momo' ? '15.000₫' :
                           activePaymentTab === 'zalopay' ? '12.000₫' :
                           activePaymentTab === 'banking' ? '10.000₫' : '25.000₫'}
                        </span>
                      </div>
                    ...
                    <form onSubmit={handleConfirmPayment} style={{ marginTop: '24px' }}>
                      <div className="checkout-agreement">
                        ...
                        <label htmlFor="checkoutAgree">Tôi xác nhận các thông tin mua sản phẩm trên là chính xác và đồng ý với điều khoản thanh toán.</label>
                      </div>
                      <button 
                        type="submit" 
                        className="btn btn-primary" 
            ...
            <div className="container">
              ...
              <div className="dashboard-layout">
                ...
                <div className="dashboard-tabs">
                  ...
                  <span className="tab-item" onClick={() => triggerToast('Lịch sử đơn bán chỉ dành cho tài khoản đã xét duyệt là Người bán.')}>Lịch sử đơn bán</span>
                  <span className="tab-item" onClick={() => triggerToast('Hồ sơ cá nhân và Cài đặt bảo mật.')}>Cài đặt tài khoản</span>
                </div>
                ...
                <div className="orders-list-wrapper">
                  {orders.length > 0 ? (
                      ...
                      <div key={order.id} className="order-log-card">
                        ...
                        <div className="order-log-body">
                          <div className="order-log-details">
                            <div className="order-game-avatar">{order.gameName.slice(0, 2).toUpperCase()}</div>
                            <div>
                              <h4 className="order-item-name">{order.itemName}</h4>
                              <div className="order-item-seller">Người bán: <strong>{order.sellerName}</strong></div>
                              <div className="order-item-qty">Số lượng: {order.qty} | Cổng thanh toán: {order.paymentMethod}</div>
                            </div>
                          </div>

                          <div className="order-log-price">
                            <div className="price-label">Tổng thanh toán</div>
                            <div className="price-val">{(order.price * order.qty).toLocaleString('vi-VN')}₫</div>
                          </div>
                        </div>

                        <div className="order-log-actions">
                          <button className="btn btn-secondary btn-sm" onClick={() => openChatWithPartner(order.sellerName, order.gameName)}>
                            💬 Trò chuyện với người bán
                          </button>
                          {order.status === 'completed' ? (
                            <button className="btn btn-outline btn-sm" style={{ borderColor: 'var(--success)', color: 'var(--success)' }} onClick={() => triggerToast('Cảm ơn bạn đã phản hồi tốt!')}>
                              Đánh giá 5★
                            </button>
                          ) : (
                            <button className="btn btn-primary btn-sm" style={{ background: '#0284c7' }} onClick={() => triggerToast(`Đơn hàng #${order.id} đang được hối thúc giao nhanh!`)}>
                              Hối thúc giao hàng
                            </button>
                          )}
                        </div>
                    ...
                    <div className="empty-orders-view">
                      ...
                      <button className="btn btn-primary btn-sm" onClick={() => pushRoute('home')}>Khám phá chợ game ngay</button>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="container">
          <div className="footer-grid">
            <div className="footer-brand">
              <h3>G2G CLONE</h3>
              <p>
                G2G Clone là nền tảng mô phỏng chợ giao dịch trò chơi điện tử trực tuyến an toàn hàng đầu. Giúp các game thủ kết nối mua bán tiền tệ, tài khoản game, thẻ game nạp tự động nhanh chóng nhất.
              </p>
              <div className="footer-socials">
                <a href="#fb" className="social-link" onClick={(e) => e.preventDefault()}>Fb</a>
                <a href="#tw" className="social-link" onClick={(e) => e.preventDefault()}>Tw</a>
                <a href="#dc" className="social-link" onClick={(e) => e.preventDefault()}>Dc</a>
                <a href="#yt" className="social-link" onClick={(e) => e.preventDefault()}>Yt</a>
              </div>
            </div>
            ...
            <div className="footer-col">
              ...
              <ul>
                ...
                <li><a href="#blog" onClick={(e) => { e.preventDefault(); triggerToast('G2G Blog'); }}>G2G Blog</a></li>
              </ul>
            </div>
            ...
            <div className="footer-col">
              <h5>Hỗ Trợ</h5>
              <ul>
                <li><a href="#help" onClick={(e) => { e.preventDefault(); triggerToast('Trung tâm trợ giúp'); }}>Trung tâm hỗ trợ</a></li>
                <li><a href="#protect" onClick={(e) => { e.preventDefault(); triggerToast('Chi tiết bảo hiểm GamerProtect'); }}>GamerProtect</a></li>
                <li><a href="#refund" onClick={(e) => { e.preventDefault(); triggerToast('Chính sách hoàn tiền'); }}>Chính sách hoàn tiền</a></li>
              </ul>
            </div>
            ...
            <div className="footer-col">
              <h5>Người Bán</h5>
              <ul>
                <li><a href="#rules" onClick={(e) => { e.preventDefault(); triggerToast('Quy định bán hàng'); }}>Quy tắc người bán</a></li>
            ...
            <div className="footer-col">
              <h5>Pháp Lý</h5>
              <ul>
                <li><a href="#terms" onClick={(e) => { e.preventDefault(); triggerToast('Điều khoản dịch vụ'); }}>Điều khoản dịch vụ</a></li>
                <li><a href="#privacy" onClick={(e) => { e.preventDefault(); triggerToast('Chính sách bảo mật'); }}>Chính sách bảo mật</a></li>
                <li><a href="#cookies" onClick={(e) => { e.preventDefault(); triggerToast('Quy tắc cookie'); }}>Cookie Policy</a></li>
              </ul>
            </div>
          </div>
          ...
          <div className="cart-drawer-body">
            {cart.length > 0 ? (
                ...
                {cart.map(item => (
                  <div key={item.cartId} className="cart-item-card-vertical">
                    <div className="cart-item-vertical-top">
                      <span className="cart-item-vertical-icon" style={{ background: item.color }}>{item.textIcon}</span>
                      <div style={{ flex: 1 }}>
                        <h5 className="cart-item-vertical-name">{item.itemName}</h5>
                        <div className="cart-item-vertical-seller">Người bán: {item.sellerName}</div>
                      </div>
                      <span className="cart-item-vertical-remove" onClick={() => removeCartItem(item.cartId)}>🗑️</span>
                    </div>

                    <div className="cart-item-vertical-bottom">
                      <div className="qty-picker sm">
                        <button className="qty-btn" onClick={() => updateCartQty(item.cartId, -1)}>-</button>
                        <span className="qty-display">{item.qty}</span>
                        <button className="qty-btn" onClick={() => updateCartQty(item.cartId, 1)}>+</button>
                      </div>
                      <div className="cart-item-vertical-price">{(item.price * item.qty).toLocaleString('vi-VN')}₫</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="empty-cart-drawer">
                ...
                <p>Giỏ hàng trống.</p>
                <button className="btn btn-secondary btn-sm" onClick={() => { setIsCartOpen(false); pushRoute('home'); }}>Tiếp tục mua hàng</button>
              </div>
            )}
          </div>
          ...
          {cart.length > 0 && (
            <div className="cart-drawer-footer">
              <div className="cart-drawer-subtotal">
                ...
                <span className="price">{getCartTotal().toLocaleString('vi-VN')}₫</span>
              </div>
              <button 
          ...
          <div className="chat-widget-header">
            <div className="chat-partner-profile">
              <div className="avatar-fallback-mini">
                {activeChatId ? chats.find(c => c.id === activeChatId)?.avatarText : 'G2'}
              </div>
              <div>
                <span className="chat-partner-name">
                  {activeChatId ? chats.find(c => c.id === activeChatId)?.partnerName : 'Chăm Sóc Khách Hàng'}
                </span>
                <div className="chat-partner-sub">Hỗ trợ: {activeChatId ? chats.find(c => c.id === activeChatId)?.game : 'Chợ G2G'}</div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <span className="chat-minimize-btn" onClick={() => setShowChatDrawer(false)}>_</span>
            </div>
          </div>
              ...
              {activeChatId ? (
                <>
                  <div className="chat-message-list">
                    {chats.find(c => c.id === activeChatId)?.messages.map((msg, index) => (
                ...
                <div className="chat-no-active">Chọn một cuộc trò chuyện để bắt đầu trao đổi chi tiết sản phẩm.</div>
              )}
            </div>
          </div>
        </div>
